...
void	FragTrap::attack(const std::string& target) {
	if (hitPoints <= 0 || energyPoints <= 0) {
		std::cout << "FragTrap " << name << " can't attack, not enough energyPoints or hitPoints! HP: " << hitPoints << " , EP: " << energyPoints << std::endl;
		return;
	}
	energyPoints--;
	std::cout << "FragTrap " << name << " attacks " << target << ", causing " << attackDamage << " points of damage! HP: " << hitPoints << " , EP: " << energyPoints << std::endl;
}

void	FragTrap::highFivesGuys(void) {
	std::cout << "FragTrap " << name << " wants to make a highfive!" << std::endl;
}